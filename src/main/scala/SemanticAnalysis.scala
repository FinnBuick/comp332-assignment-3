...
class SemanticAnalysis(val tree : SourceTree) extends Attribution {
  ...
  lazy val errors : Messages =
    collectMessages(tree) {
      ...
      case e : Expression =>
        check(e) {
          case EqualExp(l, _) if (tipe(l) != IntType() &&
                                    tipe(l) != BoolType() &&
                                    tipe(l) != UnknownType()) =>
            message(e, "type error, expecting 'int' or 'bool' found " + tipe(l))

          case LetDecl(_, e) if (tipe(e) == UnitType()) =>
            message(e, "type error, can't bind expression of type 'unit' to a variable")
        } ++
          message(
            e, "type error, expecting '" + exptipe(e) + "' found '" + tipe(e) + "'",
  ...
  val envin : SourceNode => Environment =
    attr {
      ...
      // a new scope. The one exception to this rule occurs when the block is
      // the body of a function declaration, in which case the new scope should
      // include the parameter declarations so is handled elsewhere.
      case tree.parent.pair(n, p : Block)
          if (tree.prev(n).isEmpty && !parentIsOfType[FnDecl](p)) =>
        enter(envin(p))
    ...
    attr {
      ...
      case tree.parent.pair(n @ IdnDef(i), p) =>
        p match {
          ...
          case p : FnDecl      =>
            Function(FnType(p.args.map(_.tipe),
  ...
  val exptipe : Expression => Type =
    attr {
      ...
      case tree.parent.pair(e, AppExp(l, r)) if e eq l =>
        FnType(Vector.fill(r.length)(UnknownType()), UnknownType())
      ...
      case tree.parent.pair(_ : Block, p: FnDecl) => p.optRet.getOrElse(UnitType())
    ...
    attr{
      case tree.parent.pair(e : Expression, (_ : Block | _ : Program)) => {
        ...
        e match {
          ...
          case FnDecl(IdnDef(i), _, _, b) => (from_right union (freevars_aux(e))) - i
  ...
  val freevars_aux : Expression => Set[Identifier] =
    attr {
      ...
      case FnDecl(_, pvs, _, b) =>
        freevars_aux(b) diff (pvs.map({ case ParamDecl(IdnDef(i), _) => i }).toSet)
      ...
      case IfExp(b, t, e) => freevars_aux(b) union freevars_aux(t) union freevars_aux(e)
